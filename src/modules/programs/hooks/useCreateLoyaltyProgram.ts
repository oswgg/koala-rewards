...
import { CreateProgramInput } from '@/shared/types/loyalty-program';
import { formOptions, revalidateLogic } from '@tanstack/react-form';
import { useMutation } from '@tanstack/react-query';
import { useRouter } from 'next/navigation';
import { useState } from 'react';
import { z } from 'zod';
import { supabaseProgramsService } from '../services/supabase.programs-service';
...
export function useCreateLoyaltyProgram() {
    ...
    const createProgramMutation = useMutation({
        mutationFn: async (program: CreateProgramInput) => {
            // TODO: return programService.create(program);
            return supabaseProgramsService.create(program);
        },
        onSuccess: () => {
